...
function encrypt(sentence) {
  ...
  var encryptedSentence = sentence;
  var wordCount = sentence.split(" ").length;
  var words = sentence.split(" ");
  var topBase = Math.floor(Math.random() * 26) +1;
  for (var i=1;i<27;i++) {
    if (i===topBase) {
      encryptedSentence += cipher[i] + "ㅤ";
    }
  }
  for(var i=0;i<topBase;i++) {
    encryptedSentence = doEncrypt(encryptedSentence,words,wordCount);
  }
...
function decrypt(sentence) {
  sentence = sentence.toLowerCase();
  var findTopBase = sentence.split("ㅤ");
  var repeats = 0;
  var wordCount = sentence.split(" ").length;
  var words = sentence.split(" ");
  var decryptedSentence = "";
  for (var i = 1; i<27; i ++) {
    if (findTopBase[0]===cipher[i]) {
     repeats = textToCrypt[i]; 
    }
  }
...
function doEncrypt(inputs,[words],wordCount) {
    for (var i = 0; i < wordCount; i ++) {
...
function doDecrypt(inputs,TopBase) {
